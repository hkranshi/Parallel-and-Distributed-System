...
struct s{
    ...
    int fd;
    int pno;
    s(int _id, int _time, char _gender, char _preference, int _fd, int _pno)
        : id(_id), time(_time), gender(_gender), preference(_preference), fd(_fd) , pno(_pno){}
...
void addQueue(int id, int t,char g,char p,int clsocket){
  ...
  //std :: cout << id << p << "\n"; 
        ...
        if(p=='S'){       
        single_Queue.push(s(id,t,g,p,clsocket,0));        
        }
        if(p=='D'){
        double_Queue.push(s(id,t,g,p,clsocket,0));    
        }
        if(p=='b'){
        partialsingle_Queue.push(s(id,t,g,p,clsocket,0));         
        }
        if(p=='B'){
        partialdouble_Queue.push(s(id,t,g,p,clsocket,0));      
        }
...
void assignT(int pid,int start,char gen,char pref,int i,int fd){
    ...
    if(T[i].count==0){
          ...
          if(pref=='S'|| pref=='b'){
            ...
            T[i].pids.push_back(s(pid,start,gen,pref,fd,coun));
          ...
          if(pref=='D'||pref=='B'){
            ...
            T[i].pids.push_back(s(pid,start,gen,pref,fd,p));
    ...
    while(1){
        #pragma omp parallel
        {
         ...
         for(int i=0;i<4;i++){
            ...
            size_t offset=0;
          //  char c='"';
             offset += snprintf(response + offset, sizeof(response) - offset, "%d,%d,%d,%d,%c,%d,",T[i].pids[j].id,T[i].start,T[i].game_end,T[i].court_num,T[i].pids[j].preference,T[i].pids[j].pno);
        ...
        #pragma omp critical
        {
          send(T[i].pids[j].fd, response, offset, 0);
            ...
            size_t offset=0;
             ...
             offset += snprintf(response + offset, sizeof(response) - offset, "%d,%d,%d,,,,",T[i].pids[j].id,T[i].pids[j].time+30,-1);
             if (offset >= sizeof(response)) {
            fprintf(stderr, "Buffer overflow detected!\n");
            //break;  // You may want to handle this situation appropriately
        }
...
//     // Loop through the tokens and print them
    ...
    newSocket=0;
     ...
     #pragma omp parallel for private(newSocket) num_threads(num_thread)
